...
import HrOverlay from "../Overlay/Overlay.vue";
import HrIcon from "../Icon/Icon.vue";
import HrButton from "../Button/Button.vue";
import HrInput from "../Input/Input.vue";

import { useId, useZIndex } from "@haosama-ui/hooks";
import { typeIconMap } from "@haosama-ui/utils";

defineOptions({ name: "HrMessageBox", inheritAttrs: false });
...
<template>
  <transition name="fade-in-linear" @after-leave="destroy">
    <hr-overlay v-show="(visible as Ref).value" :z-index="state.zIndex" mask>
      <div
        role="dialog"
        class="hr-overlay-message-box"
      ...
      >
        ...
        >
          <div
            ...
            ref="headerRef"
            class="hr-message-box__header"
            :class="{ 'show-close': state.showClose }"
          >
            <div class="hr-message-box__title">
              <hr-icon
                v-if="iconComponent && state.center"
                :class="{
                  [`hr-icon-${state.type}`]: state.type,
                }"
                :icon="iconComponent"
              />
              {{ state.title }}
            ...
            <button
              v-if="showClose"
              class="hr-message-box__header-btn"
              @click.stop="handleClose"
            >
              <hr-icon icon="xmark" />
            </button>
          </div>
          <div class="hr-message-box__content">
            <hr-icon
              ...
              :icon="iconComponent"
            />
            <div v-if="hasMessage" class="hr-message-box__message">
              <slot>
                <component
          ...
          <div v-show="state.showInput" class="hr-message-box__input">
            <hr-input
          ...
          <div class="hr-message-box__footer">
            ...
            <hr-button
              ...
              >{{ state.confirmButtonText }}</hr-button
            >
          </div>
        </div>
      </div>
